...
CREDENTIALS_WORKING_DIRECTORY="${CUR_DIR}/creds"
TRUSTSTORE_WORKING_DIRECTORY="${CUR_DIR}/truststore"
KEYSTORE_WORKING_DIRECTORY="${CUR_DIR}/keystore"
...
# Write passwords to file for docker-compose
echo
echo "Kafka Docker configuration requires passwords to be read in from a file. Create SSL credential files for docker-compose? [yn]"
read create_credential_files

if [ "$create_credential_files" == "y" ]; then
  mkdir -p ${CREDENTIALS_WORKING_DIRECTORY}

  echo
  echo "Keystore password: "
  read keystore_pwd
  echo ${keystore_pwd} > "${CREDENTIALS_WORKING_DIRECTORY}/keystore.cred"

  echo "Truststore password: "
  read truststore_pwd
  echo ${truststore_pwd} > "${CREDENTIALS_WORKING_DIRECTORY}/truststore.cred"

  echo "SSL key password: "
  read sslkey_pwd
  echo ${sslkey_pwd} > "${CREDENTIALS_WORKING_DIRECTORY}/sslkey.cred"
fi